...
public class Sc_SessionController : MonoBehaviour
{
    ...
    public void OnSignInButtonPress() {
        //if (DBOperation.SignIn(EmailSignIn.text, PasswordSignIn.text)) {
          //  SceneManager.LoadScene("Menu");
        //}

        //DBOperation.GetCurrentUserDataFromDatabase();
    ...
    public void OnRegisterButtonPress()
    {
        ...
        bool state = DBOperation.CreateAccount(NamesSignUp.text, LastnamesSignUp.text, EmailSignUp.text, PasswordSignUp.text, SchoolSignUp.text, int.Parse(AgeSignUp.text), grado, gender);
        if (state){       
            SceneManager.LoadScene("Menu");
        }
